...
class CoffeeNewProvider extends ChangeNotifier {
  ...
  void addFavorite(SelectedCoffeeDetail coffee) {
    var existingItemIndex = selectedFavorite.indexWhere((item) =>
        item.coffeeTypeName == coffee.coffeeTypeName &&
        item.detail.nameDetail == coffee.detail.nameDetail);

    if (existingItemIndex != -1) {
      if (!coffee.detail.isFav) {
        selectedFavorite.removeAt(existingItemIndex);
      } else {
        selectedFavorite[existingItemIndex] = SelectedCoffeeDetail(
          coffeeTypeName: coffee.coffeeTypeName,
          detail: CoffeeDetail(
            nameDetail: coffee.detail.nameDetail,
            image: coffee.detail.image,
            desc: coffee.detail.desc,
            price: coffee.detail.price,
            isFav: !coffee.detail.isFav,
          ),
        );
      }
    } else {
      if (coffee.detail.isFav) {
        selectedFavorite.add(SelectedCoffeeDetail(
          coffeeTypeName: coffee.coffeeTypeName,
          detail: CoffeeDetail(
            nameDetail: coffee.detail.nameDetail,
            image: coffee.detail.image,
  ...
  void selectCoffeeDetail(String coffeeTypeName, CoffeeDetail detail) {
    SelectedCoffeeDetail selected =
        SelectedCoffeeDetail(coffeeTypeName: coffeeTypeName, detail: detail);
